...
describe('credentialIssuerJwksRequestSchema', () => {
  it('should validate a valid request', () => {
    ...
    const result = credentialIssuerJwksRequestSchema.parse(validRequest);
    expect(result).toEqual(validRequest);
  });

  it('should allow missing optional fields', () => {
    const minimalRequest = {};
    const result = credentialIssuerJwksRequestSchema.parse(minimalRequest);
    expect(result).toEqual(minimalRequest);
  });

  it('should allow pretty as false', () => {
    const req = { pretty: false };
    const result = credentialIssuerJwksRequestSchema.parse(req);
  ...
  it('should reject pretty as string', () => {
    ...
    const result = credentialIssuerJwksRequestSchema.safeParse(req);
  ...
  it('should reject non-object values', () => {
    const result1 =
      credentialIssuerJwksRequestSchema.safeParse('not-an-object');
    expect(result1.success).toBe(false);
    const result2 = credentialIssuerJwksRequestSchema.safeParse(123);
    expect(result2.success).toBe(false);
    const result3 = credentialIssuerJwksRequestSchema.safeParse(true);
    expect(result3.success).toBe(false);
    const result4 = credentialIssuerJwksRequestSchema.safeParse([]);
    expect(result4.success).toBe(false);
  });

  it('should reject null object', () => {
    const result = credentialIssuerJwksRequestSchema.safeParse(null);
    expect(result.success).toBe(false);
  });

  it('should reject undefined object', () => {
    const result = credentialIssuerJwksRequestSchema.safeParse(undefined);
    expect(result.success).toBe(false);
  });
});
